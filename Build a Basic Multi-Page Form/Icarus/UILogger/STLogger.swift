...
struct Log: TextOutputStream {
    ...
    mutating func write(_ string: String) {
        ...
        if let handle = try? FileHandle(forWritingTo: log) {
            ...
            var dic:[String:String] = ["log": string]
            self.string += string
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: "newWrite"), object: self, userInfo: dic as? [AnyHashable : Any])
            handle.closeFile()
        } else {
